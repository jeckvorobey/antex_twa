export default {
  common: {
    brand: 'AntEx',
    loading: 'Загружаем данные...',
    retry: 'Повторить',
    details: 'Подробнее',
    submit: 'Отправить заявку',
    exchange: 'Обменять',
    yes: 'Да',
    cancel: 'Отмена',
    close: 'Закрыть',
    empty: 'Пока пусто',
    notifications: 'Уведомления',
    profile: 'Профиль',
    back: 'Назад',
    logoAlt: 'Логотип AntEx',
  },
  nav: {
    navigation: 'Основная навигация',
    home: 'Главная',
    exchange: 'Обмен',
    history: 'История',
    profile: 'Профиль',
    more: 'Еще',
    managerDashboard: 'Дашборд',
    managerChats: 'Чаты',
    managerOrders: 'Заявки',
    managerSettings: 'Настройки',
  },
  manager: {
    role: 'МЕНЕДЖЕР',
    customerFallback: 'Клиент #{id}',
    countries: {
      thailand: 'Таиланд',
      vietnam: 'Вьетнам',
      georgia: 'Грузия',
      internal: 'Внутренний обмен',
    },
    receiveMethods: {
      cash: 'Наличные',
      qrcode: 'QR code',
      bank_account: 'Банк',
      pay_services: 'Оплата услуг',
    },
    connection: {
      idle: 'Ожидание подключения',
      connecting: 'Подключение',
      online: 'На связи',
      reconnecting: 'Переподключение',
      offline: 'Нет соединения',
    },
    orderStatus: {
      new: 'Новая',
      active: 'В работе',
      done: 'Завершена',
      cancelled: 'Отменена',
      unknown: 'Статус {status}',
    },
    relativeTime: {
      now: 'сейчас',
      minutes: '{count} мин',
      hours: '{count} ч',
    },
    units: {
      kilobyte: 'КБ',
      megabyte: 'МБ',
    },
    dashboard: {
      title: 'Рабочий стол',
      subtitle: 'Текущая операционная работа',
      today: 'Сегодня',
      active: 'Активные',
      chatsLabel: 'Чаты',
      activeOrders: 'Активные заявки',
      newChats: 'Новые чаты',
      ordersToday: '+{count} заявки',
      unreadChats: '{count} новых',
      activeOrderTotals: 'Сумма активных заявок: {total}',
      queue: 'Очередь заявок',
      viewAll: 'Все заявки',
      empty: 'Активных заявок сейчас нет',
      refreshError: 'Не удалось обновить заявки. Показаны последние данные.',
    },
    chats: {
      title: 'Чаты',
      subtitle: 'Все обращения клиентов',
      unreadSummary: 'Непрочитанных: {count}',
      searchPlaceholder: 'Имя, username или номер заявки',
      conversationFallback: 'Диалог #{id}',
      filters: {
        all: 'Все',
        unread: 'Непрочитанные',
        ariaLabel: 'Фильтр чатов',
      },
      empty: {
        title: 'Новых диалогов нет',
        text: 'Новый клиент появится здесь сразу после сообщения боту.',
        unreadText: 'Все сообщения уже прочитаны.',
      },
      error: {
        title: 'Не удалось загрузить чаты',
        text: 'Проверьте соединение и повторите запрос.',
      },
    },
    chat: {
      title: 'Диалог',
      loading: 'Загрузка…',
      loadEarlier: 'Показать предыдущие',
      empty: {
        title: 'История пока пустая',
        text: 'Можно написать клиенту первым — сообщение уйдёт через бота AntEx.',
      },
      error: {
        title: 'Не удалось открыть диалог',
        text: 'Проверьте соединение и повторите запрос.',
      },
      timeline: {
        today: 'Сегодня',
        yesterday: 'Вчера',
      },
      composer: {
        attach: 'Прикрепить файл',
        placeholder: 'Сообщение…',
        send: 'Отправить сообщение',
      },
      message: {
        edited: 'изменено',
        failed: 'Не доставлено',
        forwarded: 'Переслано от {name}',
        original: 'Исходное сообщение',
      },
      actions: {
        menu: 'Действия с сообщением',
        reply: 'Ответить',
        cancelReply: 'Отменить ответ',
        forward: 'Переслать',
        copy: 'Копировать',
        copied: 'Текст скопирован',
        copyError: 'Не удалось скопировать текст',
      },
      forward: {
        title: 'Переслать сообщение',
        search: 'Поиск получателя',
        empty: 'Диалоги не найдены',
        more: 'Показать ещё',
        loadError: 'Не удалось загрузить получателей',
        confirm: 'Переслать в диалог «{name}»?',
        sent: 'Сообщение переслано',
        error:
          'Не удалось переслать сообщение. Проверьте соединение и доступность исходного сообщения.',
      },
      recorder: {
        switchToVideo: 'Переключить на видеокружочек',
        switchToVoice: 'Переключить на голосовое',
        recordVoice: 'Удерживайте для записи голоса; Enter — начать без удержания',
        recordVideo: 'Удерживайте для записи кружочка; Enter — начать без удержания',
        requesting: 'Ожидаем доступ к устройству…',
        duration: 'Длительность записи',
        locked: 'Запись зафиксирована',
        stop: 'Остановить запись',
        delete: 'Удалить запись',
        cancel: 'Отменить запись',
        send: 'Отправить запись',
        gestureHint: 'Влево — отменить · вверх — зафиксировать',
        cameraPreview: 'Предпросмотр камеры',
        videoPreview: 'Предпросмотр видеокружочка',
        voicePreview: 'Предпросмотр голосового сообщения',
        unsupported: 'Запись недоступна в этом браузере. Отправьте файл или текст.',
        permissionDenied:
          'Разрешите Telegram доступ к микрофону и камере в настройках устройства. Если открыт Telegram Web, проверьте также разрешения сайта в браузере, затем повторите запись.',
        deviceUnavailable: 'Микрофон или камера недоступны. Проверьте устройство.',
        recordingFailed: 'Не удалось записать сообщение. Попробуйте ещё раз.',
        emptyRecording: 'Запись пуста. Удерживайте кнопку дольше.',
        tooLarge: 'Запись превышает 20 МБ. Запишите более короткое сообщение.',
      },
      player: {
        play: 'Воспроизвести',
        pause: 'Пауза',
        seek: 'Позиция воспроизведения',
        speed: 'Скорость воспроизведения',
        error: 'Не удалось воспроизвести запись',
      },
      preview: {
        ready: 'Диалог готов к сообщению',
        photo: 'Фото',
        document: 'Документ',
        voice: 'Голосовое сообщение',
        video: 'Видео',
        video_note: 'Видеокружочек',
        audio: 'Аудио',
        message: 'Сообщение',
      },
      attachment: {
        photoAlt: 'Фото клиента',
        openImage: 'Открыть изображение на весь экран',
        closeImage: 'Закрыть просмотр',
        zoomImage: 'Увеличить изображение',
        fitImage: 'Вписать изображение в экран',
        downloadName: 'вложение',
        document: 'Документ',
        unavailable: 'Вложение недоступно',
      },
      notifications: {
        loadEarlierError: 'Не удалось загрузить предыдущие сообщения',
        messageError: 'Сообщение не отправлено',
        attachmentError: 'Вложение не отправлено',
        recordingError: 'Запись не отправлена. Она сохранена для повторной отправки.',
      },
    },
    profile: {
      title: 'Профиль',
      subtitle: 'Рабочее пространство менеджера',
      fallbackName: 'Менеджер',
      role: 'Менеджер AntEx',
      telegram: 'Telegram',
      realtime: {
        label: 'Связь с клиентами',
        text: 'Новые сообщения, прочтение и счётчики обновляются без перезагрузки страницы.',
      },
      notifications: {
        label: 'Уведомления',
        text: 'Пока Mini App открыта, сообщения появляются здесь мгновенно. Если приложение закрыто, бот отправит Telegram-уведомление менеджеру.',
      },
    },
    orders: {
      title: 'Заявки',
      subtitle: 'Текущая операционная работа',
      empty: {
        title: 'Активных заявок нет',
        text: 'Новые заявки появятся здесь автоматически.',
      },
      error: {
        title: 'Не удалось загрузить заявки',
        text: 'Проверьте соединение и повторите запрос.',
      },
      actions: {
        details: 'Открыть детали заявки',
        chat: 'Открыть чат клиента',
      },
      notifications: {
        chatError: 'Не удалось открыть чат клиента',
      },
    },
    orderPage: {
      title: 'Заявка',
      subtitle: 'Операционная карточка',
      error: {
        title: 'Не удалось загрузить заявку',
        text: 'Проверьте соединение и повторите запрос.',
      },
      actions: {
        chat: 'Открыть чат клиента',
        take: 'Взять в работу',
        complete: 'Завершить заявку',
        cancel: 'Отменить заявку',
      },
      notifications: {
        statusUpdated: 'Статус заявки обновлён',
        statusError: 'Не удалось изменить статус заявки',
      },
      cancelDialog: {
        title: 'Отменить заявку?',
        text: 'Статус будет сохранён, клиент получит уведомление через бота.',
      },
    },
    orderDetails: {
      title: 'Данные заявки',
      customer: 'Клиент',
      direction: 'Направление',
      sell: 'Отдаёт',
      buy: 'Получает',
      country: 'Страна',
      city: 'Город',
      receiveMethod: 'Способ получения',
      createdAt: 'Дата создания',
    },
  },
  writeAccess: {
    authenticating: {
      title: 'Проверяем сессию',
      text: 'Подождите немного — подтверждаем вход на сервере.',
    },
    reopen_required: {
      title: 'Откройте приложение заново',
      text: 'Закройте Mini App и снова откройте её через бота AntEx в Telegram. Для безопасного входа нужны новые данные запуска.',
    },
    idle: {
      title: 'Разрешите уведомления',
      text: 'Подтвердите, что бот AntEx может сообщать вам о заявках и их статусах.',
    },
    requesting: {
      title: 'Подтвердите в Telegram',
      text: 'Telegram открыл системный запрос разрешения на сообщения от бота.',
    },
    syncing: {
      title: 'Сохраняем разрешение',
      text: 'Подождите немного — подтверждаем разрешение для вашего профиля.',
    },
    allowed: {
      title: 'Разрешение получено',
      text: 'Открываем приложение.',
    },
    denied: {
      title: 'Без разрешения продолжить нельзя',
      text: 'Разрешите сообщения, чтобы получать подтверждения и изменения статуса заявки.',
    },
    unsupported: {
      title: 'Обновите Telegram',
      text: 'Эта версия Telegram не поддерживает системный запрос разрешения.',
    },
    sync_error: {
      title: 'Не удалось сохранить разрешение',
      text: 'Telegram уже подтвердил доступ. Повторите сохранение без нового запроса.',
    },
    auth_error: {
      title: 'Не удалось проверить сессию',
      text: 'Проверьте соединение и повторите вход. До подтверждения сессии доступ закрыт.',
    },
    retryPermission: 'Разрешить сообщения',
    retrySync: 'Повторить сохранение',
    retryAuth: 'Повторить вход',
    close: 'Закрыть приложение',
  },
  home: {
    title: 'Главная',
    services: 'Сервисы',
    locations: 'Выбрать страну',
    all: 'Все',
    expandRates: 'Развернуть',
    collapseRates: 'Свернуть',
    ratePrefix: 'от',
    bonus: 'Приведи друга и получи бонус',
    bonusAction: 'Подробнее',
    social: {
      title: 'Наши социальные сети',
      links: {
        reviews: {
          title: 'Отзывы ТГ',
          subtitle: 'Опыт клиентов',
        },
        news: {
          title: 'Новостной канал',
          subtitle: 'Курсы и обновления',
        },
        instagram: {
          title: 'Instagram',
          subtitle: 'Фото и сторис',
        },
        vk: {
          title: 'ВКонтакте',
          subtitle: 'Сообщество AntEx',
        },
        max: {
          title: 'Max',
          subtitle: 'Канал AntEx',
        },
        threads: {
          title: 'Threads',
          subtitle: 'Короткие заметки',
        },
      },
    },
    rateMeta: {
      market: 'СБП, перевод',
      qrcode: 'по qrcode',
      cashAndQr: 'наличными и qrcode',
    },
  },
  exchange: {
    title: 'Курсы и обмен',
    availablePairs: 'Доступные пары',
    receiveCurrency: 'Получаю',
    payAmount: 'Оплачиваю',
    aexCurrency: 'ATXG',
    aexRateText: '1 ATXG = {rate} USDT',
    rateLabel: 'Текущий курс',
    quoteUnavailable: 'Курс временно недоступен',
    receiveMethod: 'Способ получения',
    qrcodeCountries: 'По qrcode',
    receiveCountry: 'Страна получения',
    cashCities: 'Укажите город для доставки',
    trustedPhone: 'Телефон для связи',
    trustedPhonePlaceholder: '+7 999 123-45-67',
    qrcode: 'Наличные по QR',
    cash: 'Доставка наличных',
    bankAccount: 'Перевод на счет в местном банке',
    personalBankAccount: 'Перевод на счет в личном банке',
    internalExchange: 'Внутренний обмен',
    payServices: 'Оплата сервисов',
  },
  history: {
    title: 'История операций',
    empty: 'У вас пока нет заявок.',
    all: 'Все',
    active: 'Активные',
    done: 'Завершённые',
    cancelled: 'Отменённые',
    refresh: 'Обновить историю',
    repeat: 'Повторить',
    cancel: 'Отменить заявку',
    loadingMore: 'Загружаем ещё',
  },
  profile: {
    title: 'Профиль',
    version: 'Версия {version}',
    aexBalance: 'Баланс ATXG',
    referralProgram: 'Реферальная программа',
    managerHours: 'Режим работы менеджеров',
    managersWorking: 'Менеджеры сейчас работают',
    managersOffline: 'Менеджеры сейчас офлайн',
  },
  referral: {
    title: 'Реферальная программа',
    balanceLabel: 'Ваш баланс',
    yourCode: 'Ваш реферальный код',
    copyCode: 'Копировать код',
    share: 'Поделиться',
    copyLink: 'Копировать ссылку',
    invited: 'Приглашено пользователей:',
    noReferrals: 'Вы пока никого не пригласили.',
    myReferrals: 'Мои рефералы',
    myReferralsEmpty: 'Приглашённых пользователей пока нет.',
    joinedAt: 'Дата присоединения',
    rewardPercent: 'Вознаграждение',
    totalAccrued: 'Общая сумма начислений',
    history: 'История операций',
    refresh: 'Обновить',
    noTransactions: 'Операций пока нет.',
    shareText: 'Рекомендую! AntEx — один из лучших ботов для обмена.',
    howItWorks: 'Как это работает',
    instructionStep1: 'Поделиться ссылкой',
    instructionStep1Description: 'Отправьте готовую реферальную ссылку другу.',
    instructionStep2: 'Друг открывает AntEx',
    instructionStep2Description: 'Переход по ссылке закрепляет друга за вами.',
    instructionStep3: 'Друг совершает обмен',
    instructionStep3Description: 'Учитываются завершённые обменные заявки.',
    instructionStep4: 'Вы получаете ATXG',
    instructionStep4Description: 'Вознаграждение начисляется по ставке программы.',
    instructionStep5: 'Выводите баланс',
    instructionStep5Description:
      'При достижении минимальной суммы вывода вы сможете получить выплату, оформив обычный обмен на странице «Обмен».',
    instructionStep5DescriptionPrefix:
      'При достижении минимальной суммы вывода вы сможете получить выплату, оформив обычный обмен на странице ',
    instructionStep5ExchangeLink: '«Обмен»',
    instructionStep5DescriptionSuffix: '.',
    instructionReward:
      'За каждого приглашённого друга вы получаете бонус в ATXG. Чем больше друзей — тем больше заработок!',
    termsTitle: 'Основные условия',
    terms: {
      referralPercent: 'Вознаграждение',
      referralMinWithdraw: 'Минимальный вывод',
      referralMaxWithdraw: 'Максимальный вывод',
      aexRate: 'Курс ATXG',
    },
    noLimit: 'Без ограничений',
    aexRateValue: '1 ATXG = {rate} USDT',
    txType: {
      referral_reward: 'Реферальное вознаграждение',
      withdrawal: 'Вывод',
      bonus: 'Бонус',
      adjustment: 'Корректировка',
      reserved: 'Зарезервировано',
      debited: 'Списано',
      refund: 'Возврат',
    },
    reserved: 'Зарезервировано',
    directReferrals: 'Прямых рефералов: {count}',
    referralsCount: 'Прямых рефералов',
    linkCopied: 'Ссылка скопирована!',
    referralLinkLabel: 'Ваша реферальная ссылка',
    sell: 'Продать ATXG',
    sellTitle: 'Продажа ATXG',
    sellSelectOrder: 'Выберите заявку',
    sellSelectOrderPlaceholder: 'Выберите заявку для зачисления',
    sellOrderRequired: 'Выберите заявку',
    sellAmount: 'Сумма ATXG',
    sellAvailable: 'доступно {amount}',
    sellAmountPositive: 'Сумма должна быть больше нуля',
    sellAmountExceeds: 'Сумма превышает доступный баланс',
    sellConfirm: 'Продать',
  },
  order: {
    title: 'Оставить заявку',
    rateNoticeTitle: 'Курс может меняться в зависимости от суммы.',
    rateNotice: 'Менеджер подтвердит точный курс перед обменом.',
    amount: 'Сумма',
    currency: 'Получаю',
    contact: 'Telegram для связи',
    contactPlaceholder: '@telegram',
    success: 'Заявка создана',
    offlineTitle: 'Обработаем утром',
    offlineText: 'Заявку можно создать сейчас. Менеджер вернётся к ней после начала рабочего дня.',
    offlineInlineTitle: 'Обработаем утром',
    offlineInlineNotice: 'Оформить можно сейчас.',
    continue: 'Продолжить оформление',
    back: 'Вернуться назад',
    minAmountHint: 'Мин. {amount} {currency}',
  },
  status: {
    new: 'Создана',
    confirmed: 'В обработке',
    processing: 'В обработке',
    completed: 'Завершена',
    cancelled: 'Отменена',
  },
  errors: {
    generic: 'Не удалось выполнить действие. Попробуйте ещё раз.',
    auth: 'Сессия устарела. Откройте miniapp заново из Telegram.',
    order_exists: 'У вас уже есть активная заявка.',
    rate_unavailable: 'Курс временно недоступен. Попробуйте позже.',
    exchange_min_amount: 'Минимальная сумма обмена: {amount} {currency}.',
    country_required: 'Выберите страну получения.',
    country_currency_mismatch: 'Страна не соответствует выбранной валюте получения.',
    city_manager_missing: 'Для выбранного города пока не назначен менеджер.',
    unsupported_pair: 'Выбранная валютная пара пока не поддерживается.',
    trusted_contact_not_ready: 'Добавьте телефон для связи перед отправкой заявки.',
    city_required: 'Для выдачи наличными нужно выбрать город.',
    city_country_mismatch: 'Выбранный город не соответствует выбранной стране.',
    internal_payout_contract_invalid: 'Проверьте параметры внутреннего обмена.',
    min_amount: 'Минимальная сумма для обмена — {amount} {currency}',
    aex_insufficient_balance: 'Недостаточно доступных ATXG для вывода.',
    aex_withdraw_limit_not_reached:
      'Вывод ATXG станет доступен после достижения минимального лимита.',
  },
  more: {
    title: 'Еще',
    subtitle: 'Быстрые действия и ссылки',
  },
} as const;
