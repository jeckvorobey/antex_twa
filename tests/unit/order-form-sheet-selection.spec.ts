...
describe('OrderFormSheet selection sync contract', () => {
  ...
  it('refreshes manager availability before showing offline confirmation', () => {
    ...
    expect(source).toContain('<AntexBottomSheet');
    expect(source).toContain(":aria-label=\"t('common.submit')\"");
    ...
    expect(source).toMatch(
      /<AntexBottomSheet[\s\S]*@touchstart\.passive="startSheetDrag"[\s\S]*@touchmove="trackSheetDrag"[\s\S]*@touchend="finishSheetDrag"/,
    );
    expect(source).not.toContain('@touchmove.stop.prevent="trackSheetDrag"');
    expect(source).toContain(':content-style="sheetDragStyle"');
  ...
  it('keeps the submit button in the scrollable form flow', () => {
    expect(source).toMatch(
      /<div ref="sheetScrollRef" class="app-sheet__scroll">[\s\S]*<ExchangeOrderDetails[\s\S]*<AntexButton[\s\S]*{{ t\('common.submit'\) }}[\s\S]*<\/div>\s*<\/AntexBottomSheet>/,
    );
